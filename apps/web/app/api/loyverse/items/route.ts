  ...
  try {
    ...
    if (!token) {
      return NextResponse.json(
        { error: 'Loyverse API token not configured' },
        { status: 500 }
      );
    }
    ...
    const enrichedItems = await Promise.all(items.map(async (item: any) => {
      ...
      const itemModifiers = await Promise.all(
        modifierIds.map(async (modId: string) => {
          try {
            ...
            if (!modResponse.ok) {
              console.error(`[Loyverse Items API] Failed to fetch modifier ${modId}:`, modResponse.status);
              return null;
            }
      ...
      console.log(`[Loyverse Items API] Item "${item.item_name}" has ${validModifiers.length} modifiers from modifier_ids:`, modifierIds);
    ...
    console.log('[Loyverse Items API] Sample item with modifiers:', JSON.stringify(enrichedItems.find(i => i.modifiers_info && i.modifiers_info.length > 0), null, 2));
    ...
    console.error('[Loyverse Items API] Error:', error);
    return NextResponse.json(
      { error: 'Failed to fetch Loyverse items' },
      { status: 500 }
    );
  }
}
