...
export const AppShell: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Don't guard /login, /register, and /reset-password pages
  const isAuthPage =
  ...
  return (
    <div className="min-h-screen grid grid-rows-[3rem_1fr]">
      <a
        href="#main"
        className="sr-only focus:not-sr-only focus:absolute focus:top-2 focus:left-2 bg-neutral-900 text-white px-3 py-1 rounded"
      >
      ...
      <div className="min-h-0 flex flex-col">
        ...
        {isAuthPage ? (
          ...
          <div className="grid grid-cols-[14rem_1fr] gap-0">
            <LeftDock />
            <main id="main" className="min-h-[calc(100vh-3rem)]">
              <div className="max-w-7xl mx-auto px-4 lg:px-6 py-4" role="main">
                {children}
              </div>
            </main>
          </div>
