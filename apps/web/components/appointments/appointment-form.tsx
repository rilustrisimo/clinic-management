...
export function AppointmentForm({ onSuccess, onCancel, defaultDate, defaultTime, preSelectedPatient, editMode, appointmentId, initialData }: AppointmentFormProps) {
  ...
  const createMutation = useMutation({
    mutationFn: editMode && appointmentId
  ...
  const handlePatientSelect = (patient: Patient) => {
    ...
    setPatientSearch(`${patient.firstName} ${patient.lastName}${patient.mrn ? ` (${patient.mrn})` : ''}`)
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          />
          {errors.startTime && (
            <p className="mt-1 text-[10px] text-red-600 dark:text-red-400">{errors.startTime.message}</p>
          )}
        </div>
        ...
        <div>
          ...
          />
          {errors.endTime && (
            <p className="mt-1 text-[10px] text-red-600 dark:text-red-400">{errors.endTime.message}</p>
          )}
        </div>
      </div>
        ...
        <div className="space-y-3">
          ...
          {selectedItem.modifiers_info && selectedItem.modifiers_info.length > 0 ? (
            selectedItem.modifiers_info.map((modifier) => (
              <div key={modifier.id} className="rounded-lg border border-neutral-200 p-3 dark:border-neutral-700">
            ...
            <div className="rounded-lg border border-dashed border-neutral-300 bg-neutral-50 p-4 dark:border-neutral-700 dark:bg-neutral-900/50">
              <div className="text-center text-sm text-neutral-500 dark:text-neutral-400">
                ...
                <div className="text-[10px]">
                  To add modifiers: Go to Loyverse → Items → Select &quot;{selectedItem?.item_name}&quot; → Add Modifiers
                </div>
              </div>
            </div>
          )}
          ...
          <div className="rounded-lg border border-green-200 bg-green-50 p-3 dark:border-green-900 dark:bg-green-950">
            <div className="flex items-center justify-between">
              ...
              <div className="text-xl font-semibold text-green-900 dark:text-green-100">
                ₱{(() => {
                  const basePrice = selectedItem.variants.find(v => v.variant_id === currentVariantId)?.price || 0
      ...
      <div className="flex items-center justify-end gap-3 border-t border-neutral-200 pt-4 dark:border-neutral-800">
        ...
        <button
          type="submit"
          disabled={createMutation.isPending || (!editMode && !selectedPatient) || (services.length === 0 && !currentItemId)}
